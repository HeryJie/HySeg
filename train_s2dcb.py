...
class Supervision_Train(pl.LightningModule):
    def __init__(self, config):
        ...
        self.s1_net = config.s1_net
        self.s1_net_path = config.s1_net_path
        condition_s1_state_dict = torch.load(self.s1_net_path, map_location='cuda')['state_dict']
        new_state_dict_s1 = {}
        for k, v in condition_s1_state_dict.items():
            if k.startswith('net.'):
                new_state_dict_s1[k[4:]] = v
            else:
                new_state_dict_s1[k] = v
        self.s1_net.load_state_dict(new_state_dict_s1)
        self.s1_net.eval()
    ...
    def forward(self, x):
        fm_out = self.s1_net(x)
        seg_pre = self.net(x, fm_out)
        return seg_pre

    def training_step(self, batch, batch_idx):
        img, mask = batch['img'], batch['gt_semantic_seg']
        with torch.no_grad():
            fm_out = self.s1_net(img)
        prediction = self.net(img, fm_out)
